...
- Produces:
  - `plan_tick(state, now: datetime) -> list[dict]`：**纯函数，不改 state**。每个 action 形如
    `{"type": "nudge|daily_close|weekly_report|level_down", "habit_id": str|None, "stage": "remind|first|warn|final|lastcall|none", "mood": "urge|disappointed|angry|cute|celebrate", "slots": {...}}`
...
3. 当日 due 且未打卡：
   - strict 四轮（PRD §4.2"提醒 1 + 催促 3"）：窗口开启后第一个 tick → `remind`（urge）；窗口起+1h → `first`（urge）；窗口起+3h → `warn`（disappointed）；窗口结束前 30min → `final`（angry）。名义时刻落入免打扰（≥22:30）的 stage 提前至 22:30 前最后一个可发 tick，**多个积压 stage 在同一 tick 合并为最高优先级一条**（final > warn > first > remind）。已发过的 stage（`per_habit_day[h@d]["stages"]` 集合）跳过；当日 count≥4 全跳过（四轮恰好用满上限）。
   - chill 两轮：窗口中点后的第一个 tick → `remind`（cute）；窗口结束前 1h（且 <22:30，否则同上合并提前）→ `lastcall`（cute）。同 stage 幂等。
...
Expected: 全部 PASS；CLI 打印含 `"stage": "first"` 的 JSON（对 state/habits.json 空习惯则 actions 为空数组——冒烟前先把种子状态临时加一条习惯，验完还原）
...
- [ ] **Step 2: 写 references/personas.md（两档完整模板池，strict 10 条 / chill 10 条）**
...
内容（PRD §4.2 硬约束的操作员视角重述）：stage 机（strict：remind/first/warn/final 四轮；chill：remind/lastcall 两轮；含时刻表与免打扰合并提前规则）、每日 ≤4 条由脚本 nudge_state 保证、免打扰 22:30–07:00 绝对优先、幂等规则、熔断（no_response_days≥3 → level_down，M1 即生效）、chill 周目标制判定说明（week_count ≥ weekly_goal 即达标，周报呈现）。
