...
|---|---|---|---|
| `remind` | 窗口开启后第一个 tick（≥ `w_start`；`w_start` 早于 07:00 时顺延至 07:00，见 §3） | `urge` | 温和提醒 |
...
## 3. 免打扰 22:30–07:00 两段（最高优先级，可被 `user.quiet_hours` 覆盖）

- 免打扰是跨零点区间，**两段都算**：`22:30 <= t`（晚间段 22:30–24:00）或 `t < 07:00`（清晨段 00:00–07:00）。
...
- 名义时刻落入免打扰的 `nudge`：
  - **strict**：顺延/提前到免打扰外的 tick——清晨段 → 07:00（窗口开启提醒顺延，窗口起早于 07:00 时生效）；晚间段 → 22:30 前最后一个可发 tick。同一 tick 积压多个 stage 时**合并为最高优先级一条**，优先级 `final > warn > first > remind`（合并的那一条只消耗一个 stage 幂等位，被合并掉的 stage 不补发）。
  - **chill**：直接**作废**，不提前、不补发。
